...
//   ・scroll anchoring（画面外で高さが増えたぶんをエンジンが補うか）
//     Playwright の WebKit は補い、実機の WKWebView は抜けた。読み位置が保たれるかを
//     見るテストは `overflow-anchor: none` を差し込んで、エンジンの助けを借りない状態で
//     測ること（viewmode.spec.js の openBare）。切らないと実機で壊れているものが緑になる。
