...
```sh
md .                       # フォルダモード（カレントをサイドバー表示）
md docs/                   # フォルダモード（指定ディレクトリをサイドバー表示）
md intro.md                # カレント内のファイル → フォルダモードで開きそのファイルを初期表示
md ~/other/note.md         # カレント外のファイル → 単ファイルモード
md --sample                # サンプルMarkdownを stdout に出力
```

モードの選ばれ方：

- **カレント外のファイル**を指定 → 単ファイルモード（900px幅）
- それ以外（ディレクトリ指定 or カレント内のファイル指定）→ フォルダモード（1200px幅）。カレント内のファイルを指定した場合は、カレントディレクトリをルートにしてそのファイルを初期表示
