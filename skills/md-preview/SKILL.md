...
ウィンドウは **1 枚に集約される**。既に開いている状態で `md` を叩くと、新しいウィンドウは増えず既存のウィンドウにタブとして届き、そのウィンドウが前面に出る（ユーザーがコメントを書いている最中だけは、表示を奪わずタブに載せるだけになる）。タブは**いま見ているタブの右隣**に挿さり、渡した先頭がアクティブになる（既に開いているファイルなら、タブを増やさずそこへ移る）。2 枚目をどうしても開きたいときだけ `md -n <file>`（`--new-window`）を使う。
