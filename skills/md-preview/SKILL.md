...
- コメント（`c`）は raw ソース表示（⌘R）と非 md ファイルのソースビューでも使える（1 行が 1 ユニット。行番号はプレビューと同じソース行なので、一覧では両方のコメントが混ざって並ぶ。本文への印は付けた表示にだけ出る）。git 差分（⌘D）には付けられない。モード中はコメントが対象ブロックの直下にも GitHub 風に埋め込み表示される（raw / ソースビューはサイドバー一覧で読む）。
