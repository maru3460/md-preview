...
主な機能: 検索（⌘F。もう一度押すと閉じる）・フルスクリーン（⌃⌘F）・ファイル検索（⌘P。フォルダモード限定。未入力なら git で変更のあるファイルが先頭）・アウトライン（⌘T）・git 差分（⌘D）・raw ソース表示（⌘R）・本文へのコメント（`c`）・Mermaid / draw.io 図・テーマ・`.html` の iframe 描画・非 md ファイルのソースビュー。図のライブラリは図がある時だけ遅延ロードされるので通常の Markdown は軽いまま開ける。**キーボードだけで読み歩け**、フォルダを開けばファイルツリーとタブ（⇧Tab で次のタブへ・⌘W で閉じる）で複数ファイルを行き来できる（⌘B でツリーを畳んで本文を広げられる）。`?` でショートカット一覧が出る。
...
- カレントディレクトリ外のファイル（一時ディレクトリなど）を 1 つだけ → 単一ファイルモード（サイドバーなし。タブも無い）
- ディレクトリ（`md .` / `md docs/`） → そこをルートにしたフォルダモード
- ファイルを 2 つ以上 → 常にフォルダモード。ルートは全部がカレントディレクトリ配下ならそこ、そうでなければ渡したファイルたちの共通の親（ディレクトリは複数指定に混ぜられない）
...
- コメント（`c`）は raw ソース表示（⌘R）と非 md ファイルのソースビューでも使える（1 行が 1 ユニット。行番号はプレビューと同じソース行なので、一覧では両方のコメントが混ざって並ぶ。本文への印は付けた表示にだけ出る）。git 差分（⌘D）には付けられない。モード中はコメントが対象ブロックの直下にも GitHub 風に埋め込み表示される（raw / ソースビューでも行の直下に出る。1 万行を超えるソースは行コメント不可でサイドバー一覧のみ）。
- html 表示中もスクロール素キー・`?`・ファイル移動（`]` / `[` / `Tab`）・⌘F 検索は効く（検索は iframe の中身も対象。ただし cross-origin へ遷移した iframe と shadow DOM の中は除く）。素キーの `/` は iframe にフォーカスがあるときは効かない（⌘F を使う）。アウトラインは iframe の**中身**には効かない。コメント（`c`）も html 表示中は使えない。
