...
**書いたファイルが複数あるなら 1 つずつ開かせず、まとめて 1 回の `md` に渡す。** 指定した順にタブが並び、先頭が最初に表示される。ユーザーは ⇧Tab で次のタブへ移れる（分けて叩いても届くが、そのたびにウィンドウが前面に出てユーザーの手を止めるし、ユーザーが途中で別のタブへ移っていると並びが崩れる）。

ウィンドウは **1 枚に集約される**。既に開いている状態で `md` を叩くと、新しいウィンドウは増えず既存のウィンドウにタブとして届き、そのウィンドウが前面に出る（ユーザーがコメントを書いている最中と ⌘P で検索している最中だけは、表示を奪わずタブに載せるだけになる）。タブは**いま見ているタブの右隣**に挿さり、渡した先頭がアクティブになる（既に開いているファイルなら、タブを増やさずそこへ移る）。2 枚目をどうしても開きたいときだけ `md -n <file>`（`--new-window`）を使う。

届く先は**受け取り口を持っているウィンドウ**で、ふつうは最初に開いた 1 枚。そのウィンドウを閉じると、次に `md` を叩いて開くウィンドウが受け取り口を引き継ぐ。ただし**ウィンドウがある状態で** `md -n` や `md <dir>` を叩いて開いた 2 枚目は受け取り口を持たないので、そのあとの `md file.md` は目の前の新しい窓ではなく元の窓へ届く。

末尾の `&` は不要。`md` は自分をバックグラウンドへ切り離して即座に終了するので、呼び出し元がコマンドの待ち時間の上限に達してプロセスグループを畳んでも、ウィンドウは残る（ウィンドウはユーザーが ⌘W で閉じる）。

画面はどの開き方でも同じ（ディレクトリツリーのサイドバー＋タブ＋プレビュー）。開くパスで決まるのはルートの位置だけ（ルートが決まるのは窓を作るときなので、既存の窓へ届いたファイルはルートの外でもそのままタブに乗る）:

- ディレクトリ（`md .` / `md docs/`） → そこがルート。タブは開かない。**ディレクトリだけは既存の窓へ送らず、新しい窓が開く**
...
```bash
md <file.md|ディレクトリ>   # ファイルかディレクトリを開く（2 回目以降は既存の窓のタブになる）
md <a.md> <b.md> …          # 複数ファイルをタブで開く
md -n <file.md>             # 既存の窓へ送らず、新しい窓で開く
...
- `.html` / `.htm` → iframe でそのままレンダリングされる（ミニブラウザ方式）。⌘R で生ソース表示に切り替えられる。中の JavaScript は sandbox なしで実行されるので、自分が書いた HTML を見せるのは問題ないが、出所の分からない `.html` を開かせるのは避ける（`.md` も同じ扱いで、本文の script は止めていない）。
...
本文に生の HTML を書いてもそのまま描画される（サニタイズも CSP も無いので `<script>` や外部の `<iframe>`・Web フォントも動く）。
