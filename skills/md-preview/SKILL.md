...
ウィンドウは **1 枚に集約される**。既に開いている状態で `md` を叩くと、新しいウィンドウは増えず既存のウィンドウにタブとして届き、そのウィンドウが前面に出る。タブは**いま見ているタブの右隣**に挿さり、渡した先頭がアクティブになる（既に開いているファイルなら、タブを増やさずそこへ移る）。2 枚目をどうしても開きたいときだけ `md -n <file>`（`--new-window`）を使う。

届く先は**その日いちばん最初に開いたウィンドウ**に固定される。`md -n` で開いた 2 枚目と、ウィンドウがある状態で `md <dir>` を叩いて開いた窓は受け取り口を持たないので、そのあとの `md file.md` は目の前の新しい窓ではなく最初の窓へ届く。
