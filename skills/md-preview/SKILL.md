...
裸の URL はそのまま貼るだけでリンクになる（GitHub と同じ挙動）。山カッコで囲む必要はない。`http`/`https` のリンクはクリックでデフォルトブラウザが開く。同じディレクトリ内の相対リンク（`[次へ](./other.md)`）はプレビューペイン内で遷移する。ファイルパスやスキーム無しの文字列はリンクにならない。
