// VSCode 風のタブ（フォルダモード限定）。複数ファイルを開いたまま行き来する。
//
...
//
// タブの識別子は loadPreview に渡るパスそのもの（いまは root 相対）。
// root 外のファイルを絶対パスで開けるようになっても、ここは何も変えなくてよい。
(function() {
  ...
  function closeAt(i) {
    ...
    // ウィンドウを閉じる。ここで抜けてしまうと、フォルダモードでは keymap が
    // window-close を持たないので ⌘W が完全に無反応になる。
