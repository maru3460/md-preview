...
> 開いたファイルは上のタブに積まれる。`Shift+Tab` で次のタブへ（端で先頭に折り返す）、`⌘1`〜`⌘9` で n 番目、`⌘W` で閉じる（最後の 1 枚ならウィンドウが閉じる。ウィンドウを直接閉じるのは `⌘⇧W`）。タブは読み位置と Raw / 差分の状態を覚えているので、戻れば離れたところから続けられる。
> `⌘B` でファイルツリーごと畳んで本文を広げられる（畳んだ後に残る細い縦線をクリックしても戻る）。
> この段落をマウスでなぞれば、それだけでクリップボードに入る（`⌘C` は要らない。単語ならダブルクリック、段落ならトリプルクリックでもよい）。入ったことはカーソルの脇に出る小さなチェックで分かる。
> `c` でコメントモードに入り、この段落や下の表・コードをクリック（複数行はドラッグ）して指摘を付け、サイドバー（コメントタブ）の「全部コピー」でまとめてコピーできる。付けたコメントはモード中、対象の直下に GitHub 風に埋め込み表示される。マウス無しでも `j`/`k` でユニット移動・`Shift+j`/`k` でレンジ・`Enter` で付与できる。`⌘R` の raw 表示に切り替えれば、1 行単位でも付けられる。
