...
(function() {
  ...
  // 単一ファイルは root 基準が無いので Rust 側で single_file_path を使う（rel 空送信）。
  // フォルダは currentRel を送る。
  function payloadRel() {
    return mode === 'single' ? '' : (currentRel || '');
  }

  // 対象ファイルが存在するか（絶対パス/Finder/開く が使えるか）。
  function hasFile() {
    return (mode === 'folder' && !!currentRel) || mode === 'single';
  }

  // 相対パスが意味を持つか（フォルダモードでファイルがロード済みのときだけ）。
  function relCopyable() {
    return mode === 'folder' && !!currentRel;
  }
  ...
  function dispatch(action, selection) {
    switch (action) {
      case 'copy-selection':
        if (selection) navigator.clipboard.writeText(selection).catch(function() {});
        break;
      case 'copy-rel':
        if (currentRel) navigator.clipboard.writeText(currentRel).catch(function() {});
        break;
      case 'copy-abs':
        window.ipc.postMessage('menu:abs:' + payloadRel());
        break;
      case 'reveal':
        window.ipc.postMessage('menu:reveal:' + payloadRel());
        break;
      case 'open':
        window.ipc.postMessage('menu:open:' + payloadRel());
        break;
      case 'reload':
  ...
  function buildItems(selection) {
    ...
    // stdin はファイルが無いのでパス系グループごと出さない。
    if (mode !== 'stdin') {
      items.push({ label: '相対パスをコピー', action: 'copy-rel', enabled: relCopyable() });
      items.push({ label: '絶対パスをコピー', action: 'copy-abs', enabled: hasFile() });
      items.push({ sep: true });
      items.push({ label: 'Finderで表示', action: 'reveal', enabled: hasFile() });
      items.push({ label: 'デフォルトアプリで開く', action: 'open', enabled: hasFile() });
      items.push({ sep: true });
    }
  ...
  function render(items, x, y, selection) {
    close();
  ...
  document.addEventListener('contextmenu', function(e) {
    ...
    render(buildItems(selection), e.clientX, e.clientY, selection);
  });
